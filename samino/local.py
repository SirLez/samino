...
class Local(Session):
    ...
    def leave_chats(self, chatIds: Union[str, list]):
        req = self.getRequest(
            f"/x{self.comId}/s/chat/thread/leave?threadIds={','.join(map(str, chatIds))}"
        )
        return Json(req)
    ...
    def send_message(
        ...
        if message is not None and file is None:
            data["content"] = message.replace("[@", "‎‏").replace("@]", "‬‭")
        ...
        if embedImage:
            embedImage = [[100, embedImage, None]] if not isinstance(embedImage, str) else [
                [100, self.upload_media(embedImage, "image"), None]]
            data["attachedObject"] = {
                "objectId": embedId,
        ...
        if snippetLink and snippetImage:
            data.update({"attachedObject": None, "extensions": {"linkSnippetList": [
                {
                    "link": snippetLink,
                    "mediaType": 100,
        ...
        if ytVideo:
            data.update({"content": None, "type": 103, "mediaValue": ytVideo})

        if file:
            data.update({"content": None})
            if fileType == "audio":
                data.update({"type": 2, "mediaType": 110})
            elif fileType in {"image", "gif"}:
                data.update(
                    {"mediaType": 100, "mediaUploadValueContentType": f"image/{fileType}", "mediaUhqEnabled": False})
            else:
                raise TypeError(fileType)
            data.update(
                {"mediaUploadValue": b64encode(file.read()).decode(), "attachedObject": None, "extensions": None})
    ...
    def edit_profile(
        ...
        if backgroundImage:
            data["extensions"]["style"]["backgroundMediaList"].append([100, backgroundImage, None, None, None])
